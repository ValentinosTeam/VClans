...
public class ClanPrefixSubcommand extends SubCommand {
    ...
    public ClanPrefixSubcommand() {
        super("clan", "prefix", List.of("success", "too-big", "too-small", "forbidden-formatting", "invalid-characters"));
        hasToBePlayer = true;
        maxArgs = 2;
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            ...
            int length = (Math.min(clan.getName().length(), maxPrefixLength));
            return List.of(clan.getName().substring(0, length));
        } else {
            return List.of();
        }
    }
